...
def is_relevant_to_business(message):
    ...
    # Check for cybersecurity/business relevance
    for topic in CYBER_SECURITY_TOPICS:
        if topic in message_lower:
            return True, "relevant"

    # If message contains question words and is about technology/security
    ...
    # For longer messages, let AI decide
...
def get_ai_response(user_message, user_id=None):
    ...
    try:
        ...
        # Custom handling for specific question types
...
def handle_message(sender_id, user_message):
    ...
    if len(response) > 2000:
        ...
        for chunk in chunks:
            bot.send_text_message(sender_id, chunk)
    else:
...
if __name__ == '__main__':
    print(f"🛡️ Cyber Venture Chatbot Starting...")
    print(f"🌐 Website: {CYBER_VENTURE_DATA['website']}")
    print(f"📧 Contact: {CYBER_VENTURE_DATA['contact_email']}")
    print(f"✅ Handles: Cybersecurity + Business queries")
    print(f"❌ Rejects: Unrelated topics")
